...
class DictionaryLoader:
    ...
    def load_formal_dictionary() -> Set[str]:
        ...
        formal_words = {
            ...
            'таким образом', 'в соответствии с', 'на основании', 'в результате',
            'в течение', 'посредством', 'относительно', 'применительно',
            'аналогично', 'констатировать', 'свидетельствовать',
            'преобладать', 'доминировать', 'реализовывать', 'формализовать',
            'интерпретировать', 'анализировать', 'исследовать',
            'эксперимент', 'методология', 'парадигма', 'концепция',
            'теоретический', 'практический', 'эмпирический', 'фундаментальный'
        }
        return formal_words
    ...
    def load_informal_dictionary() -> Set[str]:
        ...
        informal_words = {
            ...
            'прикинь', 'обалдеть', 'клево', 'отстой', 'нормально',
            'понятно', 'ладно', 'окей', 'чувак', 'братан', 'девчонка',
            'мужик', 'тачка', 'хата', 'бабки', 'тусовка'
        }
    ...
    def load_scientific_dictionary() -> Set[str]:
        ...
        return {
            'гипотеза', 'теория', 'эксперимент', 'методология',
            'анализ', 'синтез', 'дедукция', 'индукция',
            'параметр', 'переменная', 'константа', 'коэффициент',
            'корреляция', 'регрессия', 'дисперсия', 'отклонение',
            'алгоритм', 'модель', 'система', 'структура',
            'процесс', 'результат', 'заключение', 'вывод',
            'объект', 'субъект', 'феномен', 'явление'
        }

    def load_emotional_dictionary(self) -> Set[str]:
        """Loads dictionary of emotional words."""
        return {
            'прекрасный', 'ужасный', 'восхитительный', 'отвратительный',
            'счастливый', 'грустный', 'радостный', 'печальный',
            'любимый', 'ненавистный', 'милый', 'противный',
            'великолепный', 'отвратительный', 'потрясающий', 'ужасный',
            'невероятный', 'невыносимый', 'чудесный', 'кошмарный',
            'очаровательный', 'отвратительный', 'прелестный', 'ужасный',
            'блестящий', 'ужасный', 'изумительный', 'отвратительный'
        }
