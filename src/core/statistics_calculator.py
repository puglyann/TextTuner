...
class StatisticsCalculator:
    ...
    def calculate_style_deviation(
        current_metrics: Dict[str, float], target_metrics: Dict[str, Any]
    ) -> Dict[str, Dict[str, float]]:
        """
        ...
        for metric_name, target_config in target_metrics.items():
            if metric_name in current_metrics:
                ...
                relative_diff = (
                    absolute_diff / target_value if target_value != 0 else 0.0
                )
    ...
    def calculate_overall_score(
        self, deviation_analysis: Dict[str, Dict[str, float]]
    ) -> float:
        """
    ...
    def generate_bar_chart(
        ...
        bars1 = ax.bar(
            x - width / 2, current_values, width, label="Текущий", color="blue"
        )
    ...
    def generate_statistics_report(
        self, current_metrics: Dict[str, float], target_metrics: Dict[str, Any]
    ) -> str:
        """
        ...
        for metric_name, analysis in deviation.items():
            ...
            diff_direction = (
                "ниже" if analysis["current"] < analysis["target"] else "выше"
            )
