...
class StyleAdapter:
    ...
    def generate_recommendations(
        self, metrics: StyleMetrics, target_profile: StyleProfile
    ) -> List[str]:
        """
                    ...
                    if metric_name == "lexical_diversity":
                        if diff < 0:
                            recommendations.append(
                                "Увеличьте лексическое разнообразие. "
                                "Используйте синонимы вместо повторяющихся слов."
                            )
                        else:
                            recommendations.append(
                                "Слишком высокое лексическое разнообразие "
                        ...
                        if diff < 0:
                            recommendations.append(
                                "Увеличьте формальность текста. "
                                "Используйте термины и избегайте разговорных выражений."
                            )
                        else:
                            recommendations.append(
                                "Снизьте формальность текста. "
                                "Используйте более естественные, разговорные конструкции."
                            )

                    elif metric_name == "readability_index":
                        if diff < 0:
                            recommendations.append(
                                "Упростите текст для лучшей читаемости. "
                                "Разбейте длинные предложения на более короткие."
                            )
                        else:
                            recommendations.append(
                                "Текст может быть слишком упрощенным для целевого стиля. "
                                "Добавьте более сложные конструкции."
                            )

                    elif metric_name == "sentence_length_avg":
                        if diff < 0:
                            recommendations.append(
                                "Увеличьте среднюю длину предложений. "
                                "Объединяйте короткие предложения в более сложные конструкции."
                            )
                        else:
                            recommendations.append(
                                "Слишком длинные предложения. "
                                "Разбейте сложные предложения на более простые."
                            )

                    elif metric_name == "word_length_avg":
                        if diff < 0:
                            recommendations.append(
                                "Используйте более длинные, точные слова "
                                "или специальные термины."
                            )
                        else:
                            recommendations.append(
                                "Используйте более короткие и понятные слова."
                            )

        style_specific = self._get_style_specific_recommendations(
            target_profile.name, metrics_dict
        )
    ...
    @staticmethod
    def _get_style_specific_recommendations(
        style: str, metrics: Dict[str, float]
    ) -> List[str]:
    ...
    def adapt_text(
        self, text: str, current_metrics: StyleMetrics, target_config: Dict[str, Any]
    ) -> str:
        """
            ...
            for rule in self.adaptation_rules[style]:
                try:
                    should_apply = self._check_rule_condition(
                        rule.condition, current_metrics.to_dict(), target_config
                    )
